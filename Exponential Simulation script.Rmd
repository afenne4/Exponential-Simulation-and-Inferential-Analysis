...
simvar<-data.frame(VAR=apply(samp,1,var))
...
# simsd<-data.frame(SD=apply(samp,1,sd))
# 
...
# grid.arrange(varplot1,sdplot1,ncol=2)
...
```{r}
    meanplot+
    stat_function(fun=dnorm,lwd=1.5,args=list(mean=mean(means),sd=mean(simsd)/sqrt(n)))
