...
``` {r comparing simulated mean and theoretical mean}
# Theoretical mean
Tmean<-1/lambda
...
around the theoretical mean (vertical red dashed line). The actual values of the
theoretical and simulated means are presented below. The simulated values are 
incredibly close to the theoretical value.

```{r}
meancompare<-data.frame(Mean=c("Theoretical Mean","Simulated Mean"),
                        Value=c(1/lambda,mean(simmean$means)))
meancompare
```

## Sample Variance vs. Theoretical Variance
This next chunk of code will show a plot of the variance and standard deviation
from the Monte Carlo simulation with the theoretical variance overlayed as a
dashed red vertical line. The theoretical variance for exponential 
distribution is calculated as E(Var)=1/lambda^2. 
``` {r plot simulated variance}
...
This histogram shows the right skew of the simulated variance. The theoretical
variance (dashed red line) lies to the right of the highest density in the plot.
In relation to actual values, the results below show that the average variance of
the simulations actually provides a close estimate to the theoretical value. 

```{r}
varcompare<-data.frame(Variance=c("Theoretical Variance","Average Simulated Variance"),
                        Value=c(1/lambda^2,mean(simvar$VAR)))
varcompare
```
